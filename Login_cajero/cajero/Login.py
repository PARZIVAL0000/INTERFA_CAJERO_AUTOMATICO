    ...
    class Login(Frame):
        ...
        def __init__(self,parent=""):
            ...
            self.titulo = Label(self,text="JADAC C.A EN CONFIANZA", 
                background="#0a5b98",foreground="white", font=("Cabria", 20),pady=5, anchor="center", width=41)
            ...
            self.textoID = Label(self, text="INSERTA EL IDENTIFICADOR", foreground="white", font=("Arial", 15), bg="#262b5a")
            self.textoID.grid(row=4, column=0, ipady=10, ipadx=90)
            ...
            """
            self.inputID = Entry(self, width=50, justify="center", border=3, bg="#12537d", fg="white",
            #nuestra configuracion para validar
            validate="key",
            validatecommand=(parent.register(self.verificarID),'%S')
            )
            self.inputID.grid(row=5, column=0, pady=15, ipady=7)

            self.separador = Label(self, text="\n",bg="#262b5a")
            self.separador.grid(row=6, column=0)

            self.textoPASS = Label(self, text="INSERTA LA CONTRASEÑA", foreground="white", font=("Arial", 15), bg="#262b5a")
            self.textoPASS.grid(row=7, column=0)
            self.inputPASS = Entry(self, width=50, justify="center", border=3, bg="#12537d", fg="white", show="*")
            self.inputPASS.grid(row=8, column=0, pady=15, ipady=7)
            ...
            self.boton.grid(row=10, column=0, pady=10, ipady=5)
